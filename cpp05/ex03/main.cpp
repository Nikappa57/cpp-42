...
int main(void) {
	...
	{
		try {
			std::cout << "16) Create an Intern and use it to create a form" << std::endl;
			Intern someRandomIntern;
			AForm* rrf;
			rrf = someRandomIntern.makeForm("robotomy request", "Bender");
			
			CHECK(rrf->getName(), "Robotomy Request");
			CHECK(rrf->getTarget(), "Bender");
			CHECK(rrf->getSignGradeRequired(), 72);
			CHECK(rrf->getExecGradeRequired(), 45);
			CHECK(rrf->getSigned(), false);
			std::cout << *rrf << std::endl;
			delete rrf;

			rrf = someRandomIntern.makeForm("shrubbery creation", "home");
			CHECK(rrf->getName(), "Shrubbery Creation");
			CHECK(rrf->getTarget(), "home");
			CHECK(rrf->getSignGradeRequired(), 145);
			CHECK(rrf->getExecGradeRequired(), 137);
			CHECK(rrf->getSigned(), false);
			std::cout << *rrf << std::endl;
			delete rrf;

			rrf = someRandomIntern.makeForm("presidential pardon", "home");
			CHECK(rrf->getName(), "Presidential Pardon");
			CHECK(rrf->getTarget(), "home");
			CHECK(rrf->getSignGradeRequired(), 25);
			CHECK(rrf->getExecGradeRequired(), 5);
			CHECK(rrf->getSigned(), false);
			std::cout << *rrf << std::endl;
			delete rrf;
	...
	// TEST 17
	{
		std::cout << "17) Attempt to create a form with an invalid name" << std::endl;
		try {
			Intern someRandomIntern;
			AForm* rrf;
			rrf = someRandomIntern.makeForm("SUS", "home"); // Should print an error message without throwing
			CHECK(rrf, NULL);
		} catch (std::exception &e) {
			std::cerr << "Exception: " << e.what() << std::endl;
			return fail();
